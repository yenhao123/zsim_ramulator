...
VOID FI_InjectFault_Mem(UINT32 *readAddr, UINT32 readSize) {
    ...
    // Convert readAddr to a comparable type (uintptr_t)
    uintptr_t addr = reinterpret_cast<uintptr_t>(readAddr);

    // Check if the address is within the safe range
    if (addr < safeRangeStart || addr > safeRangeEnd) {
        //std::cerr << "Error: Address " << readAddr << " is out of the safe range." << std::endl;
        return;
    }

    // Write the modified value back to memory
    UINT32 originalValue = *readAddr;
    int n_bit = 32;
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<> dis(0.0, 1.0);
    UINT32 flippedValue = originalValue;
    ...
    std::cout << "Original addr: " << readAddr << "Original value: " << std::bitset<32>(originalValue) << ", Flipped value: " << std::bitset<32>(flippedValue) << std::endl;
    PIN_SafeCopy((VOID*)readAddr, &flippedValue, sizeof(UINT32));
}
VOID MemoryAccessAnalyze(VOID * ip, VOID *memp, UINT32 size) {
    int value = *((int*)memp);  // Dereference memp to get the integer value

    std::cout << "Executing " << ip << ", memory " << memp
              << ", value " << value
              << ", in hex " << std::hex << value << std::dec << std::endl;

    // Check if the address is within the specified range
    if (reinterpret_cast<uintptr_t>(memp) >= 0x700000000000 && reinterpret_cast<uintptr_t>(memp) <= 0x7fffffffffff) {
        if (memp < lowest_memory) {
            lowest_memory = memp;
        }
        if (memp > largest_memory) {
            largest_memory = memp;
        }
    }

    if(!FUNCTION_BEGIN){
        std::cout << "Lowest memory accessed: " << lowest_memory << std::endl;
        std::cout << "Largest memory accessed: " << largest_memory << std::endl;
    }
}
        ...
        if (INS_IsMemoryRead(ins))
        {
            if (!INS_IsPredicated(ins))
            {
                ...
                if(FUNCTION_BEGIN){
                    ...
                    ins, IPOINT_BEFORE, (AFUNPTR)MemoryAccessAnalyze,
                    IARG_ADDRINT, INS_Address(ins),
                    IARG_MEMORYREAD_EA,
                    IARG_MEMORYREAD_SIZE,
                    IARG_END); 
                    */
                    //fault injection, fallthrough function e.g., branch instruction
                    if (INS_HasFallThrough(ins)) {
                        INS_InsertCall(ins, IPOINT_AFTER, (AFUNPTR)FI_InjectFault_Mem, IARG_END);
                    }
                }
...
BOOL FollowChild(CHILD_PROCESS childProcess, VOID *userData)
{
    ...
    if (procIdx == 0)
        panic("process0 cannot exec(), it spawns globally needed internal threads (scheduler and contention); run a dummy process0 instead!");
...
/*
VOID CaptureReadAddress(ADDRINT readAddr, UINT32 readSize) {
    lastReadAddr = readAddr;
    lastReadSize = readSize;
}

VOID Fault_Injection(INS ins, VOID *v)
{
    if (INS_IsMemoryRead(ins))
    {
        INS_InsertPredicatedCall(
            ins, IPOINT_BEFORE, (AFUNPTR)CaptureReadAddress,
            IARG_MEMORYREAD_EA,
            IARG_MEMORYREAD_SIZE,
            IARG_END);
        ...
        if (INS_HasFallThrough(ins)) {
            INS_InsertCall(
                ins, IPOINT_AFTER, (AFUNPTR)AfterMemoryRead,
                IARG_END);
        }
    }
}*/
...
double read_error_rate(){
    std::ifstream file("/root/ramulator-pim/zsim-ramulator/shell/canny/error_rate.txt");
    if (!file) {
        std::cerr << "Unable to open file" << std::endl;
        return -1.0; // 返回一个错误值，表示文件打开失败
    }

    double error_rate;
    file >> error_rate; // 读取文件内容

    if (file.fail()) {
        std::cerr << "Error reading the error rate from file" << std::endl;
        return -1.0; // 返回一个错误值，表示读取失败
    }
...
int main(int argc, char *argv[])
{
    ...
    // Register the memory read fault injection
    //INS_AddInstrumentFunction(Fault_Injection, nullptr);
