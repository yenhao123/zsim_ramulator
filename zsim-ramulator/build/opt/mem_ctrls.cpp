    ...
    case GETS:
        *req.state = req.is(MemReq::NOEXCL) ? S : E;
        ...
        if (print_trace)
        {
            ...
            {
                if ((!req.is(MemReq::PREFETCH_TRACE)) && (!req.is(MemReq::PREFETCH)))
                    substract[req.coreId]++;
                ...
                if (req.is(MemReq::PREFETCH_TRACE) || req.is(MemReq::PREFETCH))
                    tracefile << threadIdx[req.coreId] << " " << req.coreId << " " << pinstr << " P " << req.lineAddr << std::endl;
                else
                    tracefile << threadIdx[req.coreId] << " " << req.coreId << " " << pinstr << " L " << req.lineAddr << std::endl;
        ...
        if (print_trace)
        {
            ...
            if (!req.is(MemReq::IFETCH))
            {
                ...
                if (req.is(MemReq::PREFETCH_TRACE) || req.is(MemReq::PREFETCH))
                    ...
                    tracefile << threadIdx[req.coreId] << " " << req.coreId << " " << pinstr << " S " << req.lineAddr << std::endl;
                previous_instr[req.coreId] = 0;
            }
            futex_unlock(&traceLock);
        }
