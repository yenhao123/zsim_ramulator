...
const char* mem_addr_file = "/root/ramulator-pim/zsim-ramulator/shell/canny/tmp/memory_addresses_group.csv";
...
void canny_edge_detection(const char* input_image, const char* output_image, const char* ground_truth_image, double low_threshold, double high_threshold, int image_idx) {
    ...
    std::ofstream outf;
    outf.open(quality_file, std::ios::out | std::ios::app);
    outf << image_idx << "," << rmse << std::endl;
    outf.close();
...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " <group_number> <debug flag>" << std::endl;
        return 1;
    }
    ...
    int group_size = 10;
    int start_n_group = group * group_size;
    int end_n_group = (group+1) * group_size;
    ...
    if (std::remove(quality_file) != 0) {
        std::cerr << "Error deleting file: " << quality_file << std::endl;
    } else {
        std::cout << "File successfully deleted: " << quality_file << std::endl;
    }

    // first round
    std::string input_image = std::string(INPUT_DIR) + std::to_string(start_n_group) + ".jpg";
    std::string output_image = std::string(OUTPUT_DIR) + std::to_string(start_n_group) + ".jpg";
    std::string gd_image = std::string(GD_DIR) + std::to_string(start_n_group) + ".jpg";
    canny_edge_detection(input_image.c_str(), output_image.c_str(), gd_image.c_str(), low_threshold, high_threshold, start_n_group);

    // start tracing
    zsim_roi_begin();
    zsim_PIM_function_begin();
    for (int i = (start_n_group+1); i < end_n_group; i++){
